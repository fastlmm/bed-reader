...
# @pytest.mark.asyncio
# async def test_cloud_cloud_read1(shared_datadir):
...
# @pytest.mark.asyncio
# async def test_cloud_cloud_write(tmp_path, shared_datadir):
#     in_file = shared_datadir / "plink_sim_10s_100v_10pmiss.bed"
#     out_file = tmp_path / "out.bed"
...
# def test_cloud_cloud_overrides(shared_datadir):
#     with open_bed_cloud(shared_datadir / "some_missing.bed") as bed:
#         fid = bed.fid
#         iid = bed.iid
#         father = bed.father
#         mother = bed.mother
...
#     with pytest.raises(KeyError):
#         open_bed_cloud(
#             shared_datadir / "some_missing.bed", properties={"unknown": [3, 4, 4]}
#         )
#     with open_bed_cloud(
#         shared_datadir / "some_missing.bed", properties={"iid": None}
#     ) as bed1:
#         assert bed1.iid is None
#     with open_bed_cloud(
#         shared_datadir / "some_missing.bed", properties={"iid": []}
#     ) as bed1:
#         assert np.issubdtype(bed1.iid.dtype, np.str_)
#         assert len(bed1.iid) == 0
#         with pytest.raises(ValueError):
...
# def test_cloud_cloud_str(shared_datadir):
#     with open_bed_cloud(shared_datadir / "some_missing.bed") as bed:
#         assert "open_bed_cloud(" in str(bed)


# def test_cloud_cloud_bad_bed(shared_datadir):
#     with pytest.raises(ValueError):
#         open_bed_cloud(shared_datadir / "badfile.bed")
#     open_bed_cloud(shared_datadir / "badfile.bed", skip_format_check=True)


# def test_cloud_cloud_bad_dtype_or_order(shared_datadir):
#     with pytest.raises(ValueError):
#         open_bed_cloud(shared_datadir / "some_missing.bed").read(dtype=np.int32)
#     with pytest.raises(ValueError):
#         open_bed_cloud(shared_datadir / "some_missing.bed").read(order="X")
#     with pytest.raises(ValueError):
#         open_bed_cloud(shared_datadir / "some_missing.bed").read_sparse(dtype=np.int32)
...
# def test_cloud_cloud_properties(shared_datadir):
#     file = shared_datadir / "plink_sim_10s_100v_10pmiss.bed"
#     with open_bed_cloud(file) as bed:
#         iid_list = bed.iid.tolist()
#         sid_list = bed.sid.tolist()
#         chromosome_list = bed.chromosome.tolist()
...
# def test_cloud_cloud_c_reader_bed(shared_datadir):
#     for force_python_only, format in [(False, "csc"), (True, "csr")]:
#         bed = open_bed_cloud(shared_datadir / "some_missing.bed", count_A1=False)
...
# # def test_cloud_cloud_bed_int8(tmp_path, shared_datadir):
# #     with open_bed_cloud(shared_datadir / "some_missing.bed") as bed:
# #         for force_python_only in [False, True]:
# #             for order, format in [("F", "csc"), ("C", "csr")]:
# #                 val = bed.read(
...
# #                         force_python_only=force_python_only,
# #                     )
# #                     with open_bed_cloud(output, count_A1=count_A1) as bed2:
# #                         assert np.array_equal(
# #                             bed2.read(
# #                                 dtype="int8", force_python_only=force_python_only
# #                             ),
...
# # def test_cloud_cloud_write1_bed_f64cpp(tmp_path, shared_datadir):
# #     with open_bed_cloud(shared_datadir / "some_missing.bed") as bed:
# #         for iid_index in [0, 1, 5]:
# #             for force_python_only, format in [(False, "csc"), (True, "csr")]:
# #                 val_sparse = bed.read_sparse(
...
# #                 output = str(tmp_path / f"toydata.F64cpp.{iid_index}")
# #                 to_bed(output, val, count_A1=False)
# #                 val2 = open_bed_cloud(output, count_A1=False).read(dtype="float64")
# #                 assert np.allclose(val, val2, equal_nan=True)
# #             assert np.allclose(val_sparse.toarray(), val2, equal_nan=True)


# # def test_cloud_cloud_write1_x_x_cpp(tmp_path, shared_datadir):
# #     for count_A1 in [False, True]:
# #         with open_bed_cloud(
...
# #                     )
# #                     to_bed(output, val, properties=properties, count_A1=count_A1)
# #                     val2 = open_bed_cloud(output, count_A1=count_A1).read(dtype=dtype)
# #                     assert np.allclose(val, val2, equal_nan=True)
# #                     val_sparse = open_bed_cloud(output, count_A1=count_A1).read_sparse(
# #                         dtype=dtype, format=format
# #                     )
# #                     assert np.allclose(val, val_sparse.toarray(), equal_nan=True)


# def test_cloud_cloud_respect_read_inputs(shared_datadir):
#     import scipy.sparse as sparse
...
# def test_cloud_cloud_threads(shared_datadir):
#     ref_val_float = reference_val(shared_datadir)
#     ref_val_float2 = ref_val_float.copy()
#     ref_val_float2[ref_val_float != ref_val_float] = -127
#     ref_val_int8 = ref_val_float2.astype("int8")
...
# # def test_cloud_cloud_write12(tmp_path):
# #     # ===================================
# #     #    Starting main function
# #     # ===================================
# #     logging.info("starting 'test_writes'")
...
# #                 to_bed(filename, val, properties=properties)
# #                 for subsetter in [None, np.s_[::2, ::3]]:
# #                     with open_bed_cloud(filename) as bed:
# #                         val2 = bed.read(index=subsetter, order="C", dtype="float32")
# #                         if subsetter is None:
# #                             expected = val
# #                         else:
...
# # def test_cloud_cloud_writes_small(tmp_path):
# #     output_file = tmp_path / "small.bed"
...
# #     with open_bed_cloud(output_file) as bed:
# #         assert np.allclose(bed.read(), val, equal_nan=True)
# #         for key, value in bed.properties.items():
# #             assert np.array_equal(value, properties[key]) or np.allclose(
# #                 value, properties[key]
...
# def test_cloud_cloud_index(shared_datadir):
#     ref_val_float = reference_val(shared_datadir)
...
# def test_cloud_cloud_shape(shared_datadir):
#     with open_bed_cloud(shared_datadir / "plink_sim_10s_100v_10pmiss.bed") as bed:
#         assert bed.shape == (10, 100)


# # def test_cloud_cloud_zero_files(tmp_path):
# #     for force_python_only, format in [(False, "csc"), (True, "csr")]:
# #         for iid_count in [3, 0]:
# #             for sid_count in [0, 5]:
# #                 for dtype in [np.int8, np.float32, np.float64]:
...
# #                     # Read
# #                     with open_bed_cloud(filename) as bed2:
# #                         val2 = bed2.read(dtype=dtype)
# #                         assert np.allclose(val, val2, equal_nan=True)
# #                         val_sparse = bed2.read_sparse(dtype=dtype, format=format)
# #                         assert np.allclose(val, val_sparse.toarray(), equal_nan=True)
...
# #                     # Read again
# #                     with open_bed_cloud(filename) as bed3:
# #                         val3 = bed3.read(dtype=dtype)
# #                         assert np.allclose(val, val3, equal_nan=True)
# #                         val_sparse = bed3.read_sparse(dtype=dtype, format=format)
# #                         assert np.allclose(val, val_sparse.toarray(), equal_nan=True)
...
# def test_cloud_cloud_iid_sid_count(shared_datadir):
#     iid_count_ref, sid_count_ref = open_bed_cloud(
#         shared_datadir / "plink_sim_10s_100v_10pmiss.bed"
#     ).shape
#     assert (iid_count_ref, sid_count_ref) == open_bed_cloud(
#         shared_datadir / "plink_sim_10s_100v_10pmiss.bed", iid_count=iid_count_ref
#     ).shape
#     assert (iid_count_ref, sid_count_ref) == open_bed_cloud(
#         shared_datadir / "plink_sim_10s_100v_10pmiss.bed", sid_count=sid_count_ref
...
# def test_cloud_cloud_sample_file():
...
# # def test_cloud_cloud_coverage2(shared_datadir, tmp_path):
# #     with open_bed_cloud(
# #         shared_datadir / "plink_sim_10s_100v_10pmiss.bed", properties={"iid": None}
# #     ) as bed:
# #         assert bed.iid is None
...
# # def test_cloud_cloud_coverage3(shared_datadir, tmp_path):
# #     with pytest.warns(RuntimeWarning, match="invalid value encountered in cast"):
# #         with open_bed_cloud(
# #             shared_datadir / "small.bed", properties={"sex": [1.0, np.nan, 1.0, 2.0]}
# #         ) as bed:
...
# # def test_cloud_cloud_nones(shared_datadir, tmp_path):
# #     properties = {
# #         "father": None,
# #         "mother": None,
# #         "sex": None,
...
# #     with open_bed_cloud(shared_datadir / "small.bed", properties=properties) as bed:
# #         assert np.array_equal(bed.iid, ["iid1", "iid2", "iid3"])
# #         assert bed.father is None
...
# # def test_cloud_cloud_fam_bim_filepath(shared_datadir, tmp_path):
# #     with open_bed_cloud(shared_datadir / "small.bed") as bed:
# #         val = bed.read()
# #         properties = bed.properties
# #     output_file = tmp_path / "small.deb"
# #     fam_file = tmp_path / "small.maf"
...
# #     with open_bed_cloud(
# #         output_file, fam_filepath=fam_file, bim_filepath=bim_file
# #     ) as deb:
# #         val2 = deb.read()
# #         assert np.allclose(val, val2, equal_nan=True)
# #         val_sparse = deb.read_sparse()
...
# # def test_cloud_cloud_write_nan_properties(shared_datadir, tmp_path):
# #     with open_bed_cloud(shared_datadir / "small.bed") as bed:
# #         val = bed.read()
# #         properties = bed.properties
# #         chrom = bed.chromosome.copy()
...
# #     with open_bed_cloud(output_file) as bed2:
# #         assert np.array_equal(bed2.chromosome, ["1.0", "1.0", "5.0", "0"])
# #         assert np.array_equal(bed2.cm_position, cm_p)
...
# def test_cloud_cloud_env(shared_datadir):
#     if platform.system() == "Darwin":
#         return
...
# def test_cloud_cloud_bed_reading_example():
#     import numpy as np

#     from bed_reader import open_bed_cloud, sample_file

#     file_name = sample_file("small.bed")
#     with open_bed_cloud(file_name, count_A1=False) as bed:
#         val = bed.read(index=np.s_[:, :3], dtype="int8", order="C", num_threads=1)
#         print(val.shape)


# def test_cloud_cloud_sparse():
#     import numpy as np
...
# def test_cloud_cloud_convert_to_dtype():
#     from bed_reader._open_bed import _convert_to_dtype
