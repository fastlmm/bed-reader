...
fn writer() {
    ...
    write4(filename2, &val.view(), true, -127, 1).unwrap();
    ...
    write4(filename2, &val.view(), true, f64::NAN, 1).unwrap();
    for ext in ["fam", "bim"].iter() {
        let from = Path::new(filename).with_extension(ext);
        let to = Path::new(filename2).with_extension(ext);
        std::fs::copy(from, to).unwrap();
...
fn zeros() {
    ...
    write4(filename, &out_val01.view(), true, f64::NAN, 1).unwrap();
    ...
    write4(filename, &out_val10.view(), true, f64::NAN, 1).unwrap();
    ...
    write4(filename, &out_val00.view(), true, f64::NAN, 1).unwrap();
