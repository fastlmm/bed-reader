...
#[cfg(test)]
use crate::{
    counts, impute_and_zero_mean_snps, matrix_subset_no_alloc, read, read_with_indexes, write4,
};
#[cfg(test)]
...
fn writer() {
    ...
    write4(filename2, &val.view(), true, -127).unwrap();
    ...
    write4(filename2, &val.view(), true, f64::NAN).unwrap();
    for ext in ["fam", "bim"].iter() {
        let from = Path::new(filename).with_extension(ext);
        let to = Path::new(filename2).with_extension(ext);
        std::fs::copy(from, to).unwrap();
    ...
    if let Err(BedErrorPlus::BedError(BedError::BadValue(_))) =
        write4(filename, &val.view(), true, f64::NAN)
    {
...
fn zeros() {
    ...
    write4(filename, &out_val01.view(), true, f64::NAN).unwrap();
    ...
    write4(filename, &out_val10.view(), true, f64::NAN).unwrap();
    ...
    write4(filename, &out_val00.view(), true, f64::NAN).unwrap();
